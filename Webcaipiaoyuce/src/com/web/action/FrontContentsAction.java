			...
			for(int i = 0;i < list.size();i++){
				if(flag != null){
					...
					if(endDate.getTime() >= now.getTime()){
						list.get(i).setFlag("now");
					}else{
						list.get(i).setFlag("old");
					}
				}
			}
			request.setAttribute("list", list);
	...
	public String frontContentsPage(){
		...
		try{
			...
			for(int i = 0;i < opList.size();i++){
				...
				Date now = new Date();
				if(endDate.getTime() >= now.getTime()){
					opList.get(i).setFlag("now");
				}else{
					opList.get(i).setFlag("old");
				}
			}
			request.setAttribute("opList", opList);
			
			logger.info(">>>>>>>>>>>>>>市长信箱>>>>>>>>>>>>>>");
			Letter szxx = new Letter();
			szxx.setFlag("1");
			szxx.setIsOpen("1");
			List<Letter> szxxList = frontLetterServices.frontLetterDetail(szxx);
			request.setAttribute("szxxList", szxxList);
			
			logger.info(">>>>>>>>>>>>>>我要写信>>>>>>>>>>>>>>");
			Letter wyxx = new Letter();
			wyxx.setFlag("2");
			wyxx.setIsOpen("1");
			List<Letter> wyxxList = frontLetterServices.frontLetterDetail(wyxx);
			request.setAttribute("wyxxList", wyxxList);
			
			logger.info(">>>>>>>>>>>>>>我要建议>>>>>>>>>>>>>>");
			Letter wyjy = new Letter();
			wyjy.setFlag("3");
			wyjy.setIsOpen("1");
			List<Letter> wyjyList = frontLetterServices.frontLetterDetail(wyjy);
			request.setAttribute("wyjyList", wyjyList);
