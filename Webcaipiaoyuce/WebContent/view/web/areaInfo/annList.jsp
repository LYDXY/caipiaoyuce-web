...
<!--         <div class="main"> -->
                ...
                <div class="position"><span>当前位置：</span><a href="${ctx}/web/index.action">网站首页</a> >
					<a href="${ctx}/web/areaInfoList.action?type=announcement&catalogID=${leftCatalogList[0].theID}">
                            	通知公告
					</a>
				</div>
                <div class="list">
                    ...
                    <ul>
                        <c:if test="${catalog.catalogType=='t_contents' }">
                            <c:forEach items="${contentsList.list}" var="item">
                                    ...
                                    <li><a href="${ctx}/web/contentsShow.action?type=announcement&contentsID=${item.theID}">${item.title }</a><em>${fn:substring(item.releaseDate,0,10) }</em></li>
